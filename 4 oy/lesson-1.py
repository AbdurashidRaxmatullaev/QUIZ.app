...
def register():
    ...
    def ochir():
        if entr18.get() == '':
            ...
            label7['text'] = 'Phone number kiriting'
            label7['fg'] = 'red'            
        elif entry10.get() == '':
            label9['text'] = 'Password kiriting'
            label9['fg'] = 'red'
        elif entry12.get() == '':
            label11['text'] = 'Confirm_password kiriting'
            label11['fg'] = 'red'
        elif entry10.get() == entry12.get():
            label11['text'] = 'Password wrong'
            label11['fg'] = 'red'
        else:
    ...
    for k,v in w.items():
        ...
        for k,v in score.items():
            if v =="Tog'ri":
                ...
                notogri_javoblar +=1
        label2 = Label(frame2, text=f"Tog'ri javoblar:  {togri_javoblar}", bg ='green', fg='white')
        label2.place(x=25, y=300)

        label3 = Label(frame2, text=f"Notog'ri javoblar:  {notogri_javoblar}", bg ='green', fg='white')
        label3.place(x=25, y=330)


    if togri_javoblar >=7:
        label4 = Label(frame2, text=f"imthon topwirildi:  {notogri_javoblar}", bg ='green', fg='white', font='bold')
        label4.place(x=45, y=360)
    else:
        label4 = Label(frame2, text=f"imthon topwirilmadi:  {notogri_javoblar}", bg ='green', fg='white', font='bold')
        label4.place(x=45, y=360)
